...
public class UserControllerTest {
    ...
    void registerUserTest() throws Exception {
        UserRequestDTO userRequestDTO = UserRequestDTO.builder().name("사용자").build();

        mockMvc.perform(post(BASE_URL + "register").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(userRequestDTO)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.name").value("사용자"));
    }
    @Test
    @DisplayName("사용자 등록_실패")
    void registerUserFailTest() throws Exception {
        UserRequestDTO invalidUser = UserRequestDTO.builder().name("").build();
    ...
    void getUserByIdTest() throws Exception {
        Long userId = userService.registerUser(UserRequestDTO.builder().name("사용자").build()).getId();
